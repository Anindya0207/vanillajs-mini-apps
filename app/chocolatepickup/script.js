document.addEventListener('DOMContentLoaded', () => {
    ...
    let ROW;
    let COL;
    let defaultMatrix = [[4,1,2],[3,6,1],[1,6,6,],[3,1,2]];
    let matrix = [];
    ...
    const highLightPath = async () => {
        ...
        let index = 0, robo1Chocolate = 0, robo2Chocolate = 0;
        ...
        if(autoPlayOn) { autoPlayOn = false; }
    }
    const autoPlay = () => {
        if(autoPlayOn) {
        ...
        const _calc = (i, j1, j2, sum, path1, path2) => {
            if(i <0 || i >= ROW || j1 < 0 || j1 >= COL || j2 < 0 || j2 >= COL) return -Infinity;
            if(i == ROW - 1) {
                let cellVal = 0;
                if(j1 == j2) {
                    cellVal = matrix[i][j1];
            ...
            for(let dj1 = -1; dj1 <= 1; dj1++) {
                for(let dj2 = -1; dj2 <=1; dj2++) {
                    ...
                    _calc(i + 1, j1 + dj1, j2 + dj2, sum + cellVal1, [...path1, [i + 1, j1 + dj1]], [...path2, [i + 1, j2 + dj2]]);
                }
            }
        }
    ...
    const validateInputs = () => {
        const sanitiseInput = (val) => typeof val == 'number' && val > 0 && val <= 10;
    ...
    const renderMaze = () => {
        ...
        matrix = Array.from({length: ROW}, () => Array.from({length: COL}, () => undefined));
